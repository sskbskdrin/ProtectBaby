...
public class Utils {
    ...
    public static void print(BluetoothGatt gatt) {
        for (BluetoothGattService gattService : gatt.getServices()) {
            // -----Service的字段信息-----//
            L.d(TAG, "-->服务的 uuid:" + gattService.getUuid() + "-->type:" + Utils.getServiceType(gattService.getType()
            ) + "-->特征条数:" + gattService.getCharacteristics().size());
            // -----Characteristics的字段信息-----//
            List<BluetoothGattCharacteristic> gattCharacteristics = gattService.getCharacteristics();
            for (final BluetoothGattCharacteristic gattCharacteristic : gattCharacteristics) {
                L.i(TAG, "---->Characteristic uuid:" + gattCharacteristic.getUuid() + " permission=" + Utils
                        .getCharPermission(gattCharacteristic.getPermissions()) + " property=" + Utils.getCharPropertie
                        (gattCharacteristic.getProperties()));
                if (gattCharacteristic.getDescriptors() != null) {
                    for (BluetoothGattDescriptor dp : gattCharacteristic.getDescriptors()) {
                        L.i(TAG, "------>Descriptors uuid=" + dp.getUuid());
                    }
                }
            }
        }
